...
interface cases_list {
  text: string[]; pic: string[]
}
/**
 *
 *
...
async function Html_Serialize_Json({
  ...
  const Url = Host + url
  console.log(Url);
  const file = await Axios.get(Url).catch(e => {
    console.log(e);
    return result
  })
...
async function start() {
  ...
  async function update(row: result) {
